...
def render_current_table(df: pd.DataFrame, use_fahrenheit: bool = False):
    if df.empty:
        st.info("No current weather data.")
        return
...
def render_comparison_bar(df: pd.DataFrame, use_fahrenheit: bool = False):
    """Compare temperature + humidity per city."""
    if df.empty:
    ...
    df_melt = df.melt(
        id_vars=["city"],
        value_vars=[temp_col, "humidity_pct"],
        var_name="Metric",
        value_name="Value"
    )
...
def render_alerts(alerts_df: pd.DataFrame):
    ...
    for _, alert in alerts_df.iterrows():
        ...
        with st.expander(f"{level.upper()} | {city} - {ts}"):
            st.write(message)